...
app.get("/all", function(req, res) {
  ...
  db.zooAnimals.find({}, function(error, found) {
...
app.get("/name", function(req, res) {
  ...
  db.zooAnimals.find().sort({ name: 1 }, function(error, found) {
    // Log any errors if the server encounters one
    if (error) {
      console.log(error);
    }
...
app.get("/weight", function(req, res) {
  ...
  db.zooAnimals.find().sort({ weight: -1 }, function(error, found) {
    // Log any errors if the server encounters one
    if (error) {
      console.log(error);
    }
